...
import numpy as np
import igraph as ig
import segmentBlobs as sb
import graphFun
import sys

from datetime import datetime, timedelta

def blobs(filelist, dirs, outputprefix, adjmat):
    ...
    for i in range(0,len(inputs)):
        ...
        setTupleList = sb.segmentSpace (net=net, msr=msr, minsetsize=3)
    ...
    for i in range(0,len(inputs)):
        ...
        coupledSets = sb.connectSets(thisSetList, overlapSize)
        setGroups = sb.joinSets(coupledSets, thisSetList)
    ...
    output.write("SampleID\tTreeID\tLevel\tGeneID\tFiltered\n")
    for x in allResults:
        for y in x:
            z = map(str, y)
            output.write('\t'.join(z)+'\n')
